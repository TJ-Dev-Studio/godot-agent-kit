...
/** Parse swarm status output into structured tasks */
export function parseSwarmStatus(
  stdout: string
): { tasks: Array<{ number: number; title: string; status: string; branch?: string }> } {
  const tasks: Array<{
    number: number;
    title: string;
    status: string;
  ...
  const lines = stdout.split("\n");
  for (const line of lines) {
    // Match task lines like: "  1. Build terrain [available]" or similar patterns
    const taskMatch = line.match(
      /^\s*(\d+)[.)]\s+(.+?)\s*\[(available|claimed|complete)\]/i
    );
    if (taskMatch) {
      tasks.push({
        number: parseInt(taskMatch[1], 10),
        title: taskMatch[2].trim(),
        status: taskMatch[3].toLowerCase(),
      });
    }
  }
...
/** Parse swarm claim output */
export function parseSwarmClaim(
  stdout: string
): { task_number?: number; title?: string; branch?: string; worktree_path?: string } {
  const result: {
    task_number?: number;
    title?: string;
    branch?: string;
  ...
  const lines = stdout.split("\n");
  for (const line of lines) {
    const taskMatch = line.match(/[Cc]laimed task (\d+)/);
    if (taskMatch) result.task_number = parseInt(taskMatch[1], 10);

    const branchMatch = line.match(/[Bb]ranch[:\s]+(\S+)/);
    if (branchMatch) result.branch = branchMatch[1];

    const worktreeMatch = line.match(/[Ww]orktree[:\s]+(\S+)/);
    if (worktreeMatch) result.worktree_path = worktreeMatch[1];

    const titleMatch = line.match(/[Tt]itle[:\s]+(.+)/);
